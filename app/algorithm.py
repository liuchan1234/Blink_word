...
def should_auto_remove(report_count: int, view_count: int) -> str:
    ...
    if view_count < Limits.REPORT_MIN_VIEWS:
        return "normal"

    report_rate = report_count / view_count

    if report_rate > Limits.REPORT_REMOVE_RATE:
        return "removed"    # >10% → auto remove, manual review
    elif report_rate > Limits.REPORT_DEMOTE_RATE:
...
def compute_group_rate_limit(today_swipe_count: int) -> int:
    ...
    Uses Limits constants for tier thresholds and cooldown durations.
    """
    from app.models import Limits

    if today_swipe_count < Limits.GROUP_RATE_TIER1_COUNT:
        return 0
    elif today_swipe_count <= Limits.GROUP_RATE_TIER2_COUNT:
        return Limits.GROUP_RATE_TIER1_SECONDS
    else:
        return Limits.GROUP_RATE_TIER2_SECONDS
