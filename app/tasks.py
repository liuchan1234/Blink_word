...
from app.services.translation_service import pre_translate_hot_posts
...
def start_background_tasks():
    ...
    _tasks.append(asyncio.create_task(_run_periodic(pre_translate_hot_posts, interval=1800, name="pre_translate")))
...
async def _send_group_summaries():
    """Send daily summary to active groups. Only once per day, around 22:00 UTC."""
    now = datetime.now(timezone.utc)
    if now.hour < 22:
        return  # Not yet time

    # Use Redis flag to ensure we only send once per day
    from app.redis_client import get_redis
    r = get_redis()
    today = now.strftime("%Y-%m-%d")
    flag_key = f"group_summary_sent:{today}"
    try:
        already_sent = await r.set(flag_key, "1", nx=True, ex=86400)
        if not already_sent:
            return  # Already sent today
    except Exception:
        return  # Redis down, skip rather than double-send
    ...
    for group in groups:
        ...
        try:
            ...
            # Detect group language from the user who added the bot, fallback to "en"
            lang = "en"
            try:
                from app.services.user_service import get_user
                pool_inner = get_pool()
                async with pool_inner.acquire() as conn_inner:
                    added_by = await conn_inner.fetchval(
                        "SELECT added_by FROM groups WHERE chat_id = $1", chat_id
                    )
                if added_by:
                    adder = await get_user(added_by)
                    if adder:
                        lang = adder.lang
            except Exception:
                pass
