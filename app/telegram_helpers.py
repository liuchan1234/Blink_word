...
# Rate limit outbound Telegram API calls to avoid 429 errors
# Telegram allows ~30 msg/s per bot; we cap at 25 with 5 buffer
...
async def api_call(method: str, **params) -> dict | None:
    """Generic Telegram Bot API call with error handling and rate limiting."""
...
async def set_my_commands(commands: list[dict], scope: dict | None = None) -> bool:
    """Set bot commands menu. Scope can limit commands to specific chat types."""
    return await api_call("setMyCommands", commands=commands, scope=scope) is not None
...
async def set_webhook(url: str, secret_token: str | None = None) -> bool:
    ...
    result = await api_call(
        ...
        allowed_updates=["message", "callback_query", "message_reaction", "my_chat_member", "inline_query"],
