...
async def pre_translate_hot_posts():
    """Batch pre-translate recent hot posts."""
    pool = get_pool()
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            """
            SELECT id, content, original_lang
            FROM posts
            WHERE created_at > NOW() - INTERVAL '1 hour'
              AND (
                  COALESCE((SELECT SUM((value)::int) FROM jsonb_each_text(reactions)), 0)
                  + like_count
                  + dislike_count
                  + favorite_count
                  + report_count
    ...
    for row in rows:
        try:
            await pre_translate_hot_post(row["id"], row["content"], row["original_lang"])
        except Exception as e:
            logger.warning("Pre-translate failed for post %s: %s", row["id"], e)

    logger.info("Pre-translated hot posts batch: %d", len(rows))
