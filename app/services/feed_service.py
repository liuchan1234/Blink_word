...
# Redis key patterns
VIEWED_KEY = "viewed:{user_id}"
CURRENT_POST_KEY = "cur_post:{user_id}"
SWIPE_COUNT_KEY = "swipes:{user_id}"
...
async def get_next_card(
    ...
    # Increment user's personal views stat
    from app.services.user_service import increment_stat
    await increment_stat(user_id, "views_total")
...
async def _mark_viewed(user_id: int, post_id: str):
    ...
    await set_add(VIEWED_KEY.format(user_id=user_id), post_id, ttl=Limits.VIEWED_TTL)
...
async def _set_current_post(user_id: int, post_id: str):
    ...
    try:
        r = get_redis()
        await r.set(CURRENT_POST_KEY.format(user_id=user_id), post_id, ex=Limits.CURRENT_POST_TTL)
    except Exception as e:
...
async def _increment_swipe_count(user_id: int) -> int:
    ...
    try:
        ...
        pipe = r.pipeline()
        pipe.incr(key)
        pipe.expire(key, Limits.SWIPE_COUNT_TTL)
